...
When merging native Prometheus output, the exporter normalizes the legacy
`cometbft` prefix to `tendermint` and Pharos's legacy `mytumbler_` consensus
prefix to `multi_proposer_consensus_`.
