    ...
    protected async updateEvmAddressBalance(addresses: string): Promise<void> {
        ...
        */
        ...
        for (const address of evmAddresses) {
            ...
            this.availableGauge.labels(address, 'WBTC').set(wbtc.amount * Math.pow(10, this.decimalPlaces));

            const weth = await this.getERC20Amount('0x6E1E9896e93F7A71ECB33d4386b49DeeD67a231A', address);
            this.availableGauge.labels(address, 'WETH').set(weth.amount * Math.pow(10, this.decimalPlaces));
