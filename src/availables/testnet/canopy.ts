    ...
    protected async updateRank(validator: string): Promise<void> {
        try {
            ...
            const normalizedValidator =
                (validator || '').toLowerCase().replace(/^0x/, '');
            const idx = sorted.findIndex((v) =>
                String((v as any).address || '').toLowerCase().replace(/^0x/, '') === normalizedValidator
            );
            ...
            this.rivalsPowerGauge
                .labels('above')
                .set(Number((above as any)?.stakedAmount ?? 0) / Math.pow(10, this.decimalPlaces));
            this.rivalsPowerGauge
                .labels('below')
                .set(Number((below as any)?.stakedAmount ?? 0) / Math.pow(10, this.decimalPlaces));
        } catch (e) {
    ...
    private async updateProposalsCount(): Promise<void> {
        try {
            ...
            const count =
                ...
                (data && data.proposals && typeof data.proposals === 'object') ? Object.keys(data.proposals).length :
