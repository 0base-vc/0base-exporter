...
export default class Story extends Tendermint {
  ...
  protected async updateRank(validator: string): Promise<void> {
    ...
    return this.get(url, (response) => {
      ...
      this.rivalsPowerGauge.labels("above").set(parseInteger(above.tokens));
      this.rivalsPowerGauge.labels("below").set(parseInteger(below.tokens));
    });
  }
  ...
  protected async updateValidatorsPower(): Promise<void> {
    ...
    return this.get(url, (response) => {
      ...
      validators.forEach((validator: any) => {
        this.validatorsGauge.labels(validator.operator_address).set(parseInteger(validator.tokens));
      });
    });
  }
  ...
  protected async getEVMAmount(address: string): Promise<{
    ...
    try {
      const amount = toDecimal(
        (await this.web3.eth.getBalance(address)).toString(),
        this.decimalPlaces,
      );
      return {
        amount,
