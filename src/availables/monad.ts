...
import { getDecimalPlaces, getEvmApiUrl, getMonadValidatorId } from "../core/runtime-env";

function toSafeNumber(value: bigint | string | number | undefined): number {
...
export default class Monad extends TargetAbstract {
  ...
  private readonly validatorIdEnv: string | undefined = getMonadValidatorId();
  private readonly validatorAbi: any = [
    ...
    },
    {
      ...
      name: "EpochChanged",
      inputs: [
        { name: "oldEpoch", type: "uint64", indexed: false, internalType: "uint64" },
        { name: "newEpoch", type: "uint64", indexed: false, internalType: "uint64" },
      ],
      anonymous: false,
    },
    {
      ...
      inputs: [
        { name: "validatorId", type: "uint64", indexed: true, internalType: "uint64" },
        { name: "delegator", type: "address", indexed: true, internalType: "address" },
        { name: "withdrawId", type: "uint8", indexed: false, internalType: "uint8" },
        { name: "amount", type: "uint256", indexed: false, internalType: "uint256" },
        { name: "activationEpoch", type: "uint64", indexed: false, internalType: "uint64" },
      ],
      anonymous: false,
    },
    {
      ...
      inputs: [
        ...
        { name: "commission", type: "uint256", indexed: false, internalType: "uint256" },
      ],
      anonymous: false,
    },
    {
  ...
  protected async updateValidatorRewards(validatorAddress: string): Promise<void> {
    try {
      ...
      const unclaimed: bigint = BigInt(
        res.unclaimedRewards?.toString?.() ?? res.unclaimedRewards ?? 0,
      );
      ...
      // res.consensusStake
      const consensusStake: bigint = BigInt(
        res.consensusStake?.toString?.() ?? res.consensusStake ?? 0,
      );
      ...
      const consensusCommission: bigint = BigInt(
        res.consensusCommission?.toString?.() ?? res.consensusCommission ?? 0,
      );
      ...
      // res.snapshotStake
      const snapshotStake: bigint = BigInt(
        res.snapshotStake?.toString?.() ?? res.snapshotStake ?? 0,
      );
      ...
      const snapshotCommission: bigint = BigInt(
        res.snapshotCommission?.toString?.() ?? res.snapshotCommission ?? 0,
      );
      ...
      this.validatorFlagsGauge.labels(validatorAddress).set(toSafeNumber(flags));

      // res.accRewardPerToken
      const accRewardPerToken: bigint = BigInt(
        res.accRewardPerToken?.toString?.() ?? res.accRewardPerToken ?? 0,
      );
  ...
  private getValidatorContract() {
    if (!this.validatorContract) {
      this.validatorContract = new this.web3.eth.Contract(
        this.validatorAbi as any,
  ...
  protected async getEVMAmount(address: string): Promise<{ amount: number }> {
    try {
      return {
        amount: await this.evmClient.getNativeBalance(address, this.decimalPlaces),
      };
    } catch (e) {
      console.error(e);
      return { amount: 0 };
    ...
    for (const address of evmAddresses) {
      try {
        ...
        while (!isDone) {
          ...
          isDone = res.isDone;
          startValId = toSafeNumber(res.nextValId?.toString() ?? res.nextValId ?? 0);
          ...
          for (const valId of res.valIds || []) {
            try {
              ...
              const stake: bigint = BigInt(
                delegatorRes.stake?.toString?.() ?? delegatorRes.stake ?? 0,
              );
        ...
        for (let withdrawId = 0; withdrawId < 3; withdrawId++) {
          try {
            const res = await contract.methods
              .getWithdrawalRequest(validatorId, address, withdrawId)
              .call();
            const withdrawalAmount: bigint = BigInt(
              res.withdrawalAmount?.toString?.() ?? res.withdrawalAmount ?? 0,
            );
  ...
  protected async updateEpoch(): Promise<void> {
    try {
      ...
      this.epochGauge.set(toSafeNumber(epoch));

      // inEpochDelayPeriod
      const inDelayPeriod =
        res.inEpochDelayPeriod === true ||
        res.inEpochDelayPeriod === "true" ||
        res.inEpochDelayPeriod === 1;
  ...
  protected async updateAddressDelegatorRewards(addresses: string): Promise<void> {
    ...
    for (const address of evmAddresses) {
      try {
        ...
        // unclaimedRewards
        const unclaimedRewards: bigint = BigInt(
          res.unclaimedRewards?.toString?.() ?? res.unclaimedRewards ?? 0,
        );
  ...
  private async getAllValidatorsFromSet(contract: any, methodName: string): Promise<string[]> {
    ...
    while (!isDone) {
      try {
        ...
        isDone = res.isDone;
        startIndex = toSafeNumber(res.nextIndex?.toString() ?? res.nextIndex ?? 0);
  ...
  protected async updateProposerValId(): Promise<void> {
    try {
      ...
      this.proposerValIdGauge.set(toSafeNumber(proposerValId));
    } catch (e) {
